...
//printf("FrameCache::put_frame %d position=" _LD "\n", __LINE__, position);
...
// printf("FrameCache::frame_exists %d item=%p item->position=" _LD " position=" _LD "\n",
// __LINE__,
// item,
...
// 	lock->lock("FrameCache::dump");
	...
	while( item ) {
 		printf("  position=" _LD " frame_rate=%f age=%d size=" _LD "\n", 
