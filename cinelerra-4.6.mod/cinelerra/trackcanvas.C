...
// track context menu to appear
	...
	BC_Pixmap *auto_pixmaps[AUTOMATION_TOTAL] = 
	{
		0, 0, 0, 0, 0, 0, 0, 0,
		pankeyframe_pixmap,
		modekeyframe_pixmap,
...
//if(ctrl_down())
//printf("TrackCanvas::test_floatauto %d cursor_x=%d cursor_y=%d in_x1=%d in_x2=%d in_y=%d in_y1=%d in_y2=%d position=" _LD "\n", 
...
// Snap to nearby values
			...
			if(shift_down())
			{
				double value1 = 0.;
				double value2 = 0.;
				double distance1 = percentage_to_value(0., 0, 0);
				double distance2 =  percentage_to_value(1., 0, 0);
...
int TrackCanvas::update_drag_pluginauto(int cursor_x, int cursor_y)
{
	...
	if(get_drag_values(&percentage, &position, 1,
		cursor_x, cursor_y, current)) return 0;
...
// Test plugin boundaries
				...
				if(do_plugins(cursor_x, cursor_y, 
					0, 1, update_cursor, rerender))
				{
					break;
				}
				else
