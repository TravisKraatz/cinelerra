...
// For the ice1712 the buffer must be maximum or no space will be allocated.
			if(device->idriver == AUDIO_OSS_ENVY24) buffer_info = 0x7fff000f;
...
// For the ice1712 the buffer must be maximum or no space will be allocated.
			if(device->odriver == AUDIO_OSS_ENVY24) buffer_info = 0x7fff000f;
...
{
	return dsp_out[number];
}

int AudioOSS::get_input(int number)
{
	return dsp_in[number];
}
